...
class AlarmStorageTest : public ::testing::Test {
protected:
    void SetUp() override {
...
TEST_F(AlarmStorageTest, CanAddAlarmLevelInformation) {
    const auto alarm_settings = alarm_settings_t(1, 1, alarmSeverity::alarm, alarmThreshold::automaticBasic);

    alarm_storage->add_alarm_level_history_item(time_point_t(100s), alarm_settings, 123.0);
    alarm_storage->add_alarm_level_history_item(time_point_t(200s), alarm_settings, 456.0);

    const auto retrieved_history = alarm_storage->get_alarm_level_history(alarm_settings);
...
TEST_F(AlarmStorageTest, CanAddAlarmFiringInformation) {
    execute_setup_statements<1>({R"(INSERT INTO channels (channel_units, channel_name, machine_id) VALUES ("g", "C1", 1))"});

    alarm_storage->add_alarm_activation(alarm_activation_t(1, 1, alarmSeverity::alert, time_point_t(100s), false));
    alarm_storage->add_alarm_activation(alarm_activation_t(1, 1, alarmSeverity::alarm, time_point_t(110s), true));
    alarm_storage->add_alarm_activation(alarm_activation_t(1, 1, alarmSeverity::alert, time_point_t(120s), false));

    const auto retrieved_history = alarm_storage->get_activations_for_machine(1); // Need to link the channel to the machine somehow
...
TEST_F(AlarmStorageTest, CanRetrieveAlarmSettings) {
    ...
    const auto retrieved_alarm_settings = alarm_storage->get_alarm_settings_for_machine(2);
    ASSERT_EQ(8, retrieved_alarm_settings.size());
}

TEST_F(AlarmStorageTest, CanUpdateAlarmSettings) {
    execute_setup_statements<10>({R"(INSERT INTO machines (machine_name, site_id) VALUES ("M1", 1))",
                                  R"(INSERT INTO machines (machine_name, site_id) VALUES ("M2", 1))",
                                  R"(INSERT INTO channels (channel_name, channel_units, machine_id) VALUES ("C1", "g", 1))",
                                  R"(INSERT INTO channels (channel_name, channel_units, machine_id) VALUES ("C2", "g", 2))",
    ...
    const auto x = alarm_storage->update_alarm_setting(alarm_settings_t(2, 2, alarmSeverity::alarm, alarmThreshold::Custom, 500.f));
    const auto retrieved_alarm_settings = alarm_storage->get_alarm_settings_for_machine(2);
