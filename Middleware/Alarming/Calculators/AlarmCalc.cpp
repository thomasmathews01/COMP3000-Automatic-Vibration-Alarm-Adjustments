...
std::pair<alarm_state_t, time_point_t> AlarmCalc::get_state_of_alarm_fixed(const time_point_t& time) {
    const auto limit =
            alarm_settings.threshold == alarmThreshold::Custom ? alarm_settings.customLevel.value() : 100; // TODO: Replace with the actual ISO level from the vibration handbook.
