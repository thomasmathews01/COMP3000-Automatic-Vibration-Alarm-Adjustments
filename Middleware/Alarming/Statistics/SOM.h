...
constexpr auto square_difference = [](const auto first, const auto second) {
    return square(difference(first, second));
}; // TODO: Move to STL Extensions

template<class T, int N>
float euclidean_distance(const std::array<T, N> &first, const std::array<T, N> &second) {
    return sqrt(inner_product(first, second, 0.f, std::plus<>(), square_difference));
}
...
public:
    ...
    auto lowest_distance_from(const som_point_t &point) {
        return [&point, this](const auto &first, const auto &second) {
            return euclidean_distance<float, FeatureDims>(first, point) <
                   euclidean_distance<float, FeatureDims>(second, point);
        };
    }
    ...
    void
    train_on(const std::vector<som_point_t> &points, const std::function<float(float, int)> &neighbourhood_function,
    ...
    void update_net_based_on_bmu(typename net::iterator bmu, const som_point_t &point,
        ...
        for (int i = 0; i < nodes->size(); ++i) {
            const auto distance_from_bmu = euclidean_distance<int, 2>(get_idx_as_coords(idx), get_idx_as_coords(i));
