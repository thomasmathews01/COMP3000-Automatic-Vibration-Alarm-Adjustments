...
private:
	...
	static constexpr auto minimum_model_representation = 0.95; // If less than 95% of points are accounted for by the model, we need to retrain_model the model.
	...
	std::vector<std::map<int , int>> state_interpretation;
	...
	void retrain_model() noexcept;
	[[nodiscard]] int get_current_state() const noexcept;
	void publish_current_state(int state, time_point_t time) const noexcept;
	...
	map_type_t::som_point_t get_som_point_representing_latest_values() const noexcept;
};
